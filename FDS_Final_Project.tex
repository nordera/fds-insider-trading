...
\paragraph{Key findings.} Opportunistic buying predicts 0.35\% higher next-month returns ($p<0.001$), or $\sim$4.2\% annualized, using strictly public information (filing dates). Routine trades remain significantly positive (0.0044, $p<0.001$). A binary opportunistic indicator is negative ($-$0.0021), confirming that direction --- not occurrence --- drives predictability. In Model 3, the direct opportunistic effect vanishes while the interaction with volatility is 0.0203 ($p<0.001$): all predictive content is concentrated in high-uncertainty environments. Comparing to transaction-date results, the slight attenuation confirms that a small portion of the raw signal reflects information not yet public at month-end --- a reassuring robustness check.
